...
public class DetonatorHeatwave : DetonatorComponent {
	...
	void Update () 
	{
		...
		if (_heatwave)//No entrara fins que SystemInfo.supportsImageEffects sigui verdader
		{
	...
	override public void Explode()
	{
        ...
        if (SystemInfo.supportsImageEffects)
        {
            ...
            if (!_delayedExplosionStarted)
            {
                _explodeDelay = explodeDelayMin + (Random.value * (explodeDelayMax - explodeDelayMin));//COMPROVAR si el resultat es 0 o menor provocara una ona de calor
            }
            if (_explodeDelay <= 0)//On es provoca l'ona de calor
            {
